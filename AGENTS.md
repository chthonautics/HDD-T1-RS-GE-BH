...
- `entities/` — MongoDB documents and domain types (`Shipment`,
  `ShipmentRequest`, `Status`, `WeightCategory`)
...
- A shipment's `Status` follows a fixed flow:
  ...
  for weighing a `HEAVY` package (status → `PESADO`), throwing
...
- `PATCH /shipments/{id}` — transition a shipment to the `Status` in the
  request body; validated against the rules above.
